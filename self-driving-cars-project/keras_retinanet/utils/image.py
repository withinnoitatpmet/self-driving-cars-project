...
def read_image_bgr(path):
    image = np.asarray(Image.open(path).convert('RGB')).astype(keras.backend.floatx())
    #print(image.shape)
    bgr_part = image[:, :, ::-1]
    #print(bgr_part.shape)
    #lidar_part = image[:, :, 0:1]
    lidar_part = np.expand_dims(np.asarray(Image.open(path.replace('_image.jpg', '_lidar.png'))), 2)
    
    #print(bgr_part.shape)
    #print(lidar_part.shape)
    #print(lidar_part.shape)
    merged_lidar = np.concatenate((bgr_part,lidar_part), -1)
    #print(merged_lidar.shape)
    return merged_lidar.copy()
