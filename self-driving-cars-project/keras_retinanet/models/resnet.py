...
def ResNet50RetinaNet(inputs, num_classes, weights='imagenet', *args, **kwargs):
    image = keras.layers.Lambda(lambda x : x[:,:,:,0:3])(inputs)
    depth_1d = keras.layers.Lambda(lambda x : x[:,:,:,3:4])(inputs)
    depth = keras.layers.Concatenate()([depth_1d, depth_1d, depth_1d])
    ...
    image_placeholder = keras.layers.Input((None, None, 3))
    depth_placeholder = keras.layers.Input((None, None, 3))
    
    resnet1 = keras_resnet.models.ResNet50(image_placeholder, include_top=False, freeze_bn=True)(image)
    resnet2 = keras_resnet.models.ResNet50(depth_placeholder, include_top=False, freeze_bn=True)(depth)

    all_output = []
    all_output.append(keras.layers.Concatenate()([resnet1[0], resnet2[0]]))
    all_output.append(keras.layers.Concatenate()([resnet1[1], resnet2[1]]))
    all_output.append(keras.layers.Concatenate()([resnet1[2], resnet2[2]]))
    all_output.append(keras.layers.Concatenate()([resnet1[3], resnet2[3]]))
    
    new_model = keras.models.Model(inputs=inputs, outputs=all_output)
    
    model = retinanet.retinanet_bbox(inputs=inputs, num_classes=num_classes, backbone=new_model, *args, **kwargs)
